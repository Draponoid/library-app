...
`GET /api/loans?status=all|active|returned|overdue`: по умолчанию all; active включает все незавершённые выдачи, в том числе просроченные; returned — возвращённые; overdue — незавершённые со сроком ранее текущей даты. Неизвестное или пустое значение даёт 422. Фильтр не отменяет ограничение читателя собственными выдачами.
